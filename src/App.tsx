...
const BUILDER_CODE_LENGTH_HEX = BUILDER_CODE.length.toString(16).padStart(2, '0');
const SCHEMA_ID_HEX = '00';
const ERC8021_MARKER = '80218021802180218021802180218021';
const ATTRIBUTION_SUFFIX = `${BUILDER_CODE_HEX}${BUILDER_CODE_LENGTH_HEX}${SCHEMA_ID_HEX}${ERC8021_MARKER}`;
...
const RARITY_HIERARCHY: Rarity[] = ['Common', 'Rare', 'Epic', 'Legendary', 'Mythic'];
...
function ClickerArea() {
  ...
  const handleClick = () => {
    ...
    if (isConnected && Math.random() > 0.95) { // 5% chance to prompt on-chain save
      toast('Save Progress On-Chain?', {
        action: {
          ...
          onClick: () => {
            ...
            const calldataWithAttribution = `${data}${ATTRIBUTION_SUFFIX}` as `0x${string}`;
            
            sendTransaction({
              to: FREN_FACTORY_ADDRESS as `0x${string}`,
              data: calldataWithAttribution,
            }, {
              onSuccess: () => toast.success('Progress saved securely on Base!'),
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between bg-white/5 backdrop-blur-lg border border-white/10 rounded-[32px] p-5 relative overflow-hidden group">
      <div>
        <h3 className="text-2xl font-black text-center tracking-tight bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent uppercase italic">Goo Generator</h3>
        <p className="text-center font-medium text-white/50 text-xs mt-1">Click the giant Goo drop to produce resources.</p>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center relative py-6">
        <motion.div animate={{ scale: clickScale }} transition={{ type: 'spring', stiffness: 400, damping: 10 }}>
          <button 
            onClick={handleClick}
        ...
        <div className="mt-12 w-full max-w-xs space-y-2 text-center bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4">
          <p className="text-[10px] uppercase font-bold tracking-widest text-white/50">Current Goo</p>
          <p className="text-4xl font-black font-mono text-cyan-400">{Math.floor(state.goo).toLocaleString()}</p>
          <p className="text-[10px] text-green-400 font-bold">+{state.frens.reduce((acc, f) => acc + f.productionRate, 0)} per sec</p>
        </div>
      </div>
    </div>
  );
...
function FactoryArea() {
  ...
  const handleHatch = () => {
    if (state.goo >= hatchCost) {
      dispatch({ type: 'HATCH_FREN', payload: { cost: hatchCost, newFren: generateFren(hatchCost) } });
  ...
  const handleMerge = (fren: Fren) => {
    ...
    const partner = state.frens.find(f => f.id !== fren.id && f.rarity === fren.rarity);
    if (!partner) {
    ...
    dispatch({ type: 'MERGE_FRENS', payload: { id1: fren.id, id2: partner.id, newFren } });
  ...
  return (
    <div className="flex flex-col h-full bg-white/5 backdrop-blur-2xl border border-white/20 rounded-[48px] relative overflow-hidden group p-6">
       ...
       <div className="relative z-10 flex items-center justify-between mb-6">
          <span className="font-bold text-xl tracking-tight text-white uppercase italic">Fren Factory Line</span>
          <button onClick={handleHatch} disabled={state.goo < hatchCost} className="px-6 py-2 bg-[#0052FF] hover:bg-[#1a66ff] disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl font-bold text-sm border-b-4 border-blue-800 transition-all text-white">
        ...
        <ScrollArea className="h-[400px] w-full pb-4">
          {state.frens.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-white/40 p-8 border border-dashed border-white/20 rounded-3xl min-h-[300px]">
              ...
              <p className="font-medium text-[10px] uppercase tracking-widest text-center">Your factory is empty.<br/>Produce Goo to hatch your first Fren!</p>
            </div>
          ) : (
                ...
                {state.frens.map((fren) => (
                  ...
                  >
                    <div className="bg-white/10 rounded-3xl border border-dashed border-white/20 flex flex-col items-center justify-center p-4 relative group hover:bg-white/20 transition-all min-h-[160px]">
                       <span className={`absolute top-3 right-3 text-[9px] font-bold px-2 py-0.5 rounded-full ${RARITY_COLORS[fren.rarity]}`}>{fren.rarity.toUpperCase()}</span>
                       <div className={`w-16 h-16 rounded-full mb-2 flex items-center justify-center text-3xl shrink-0 border border-white/10 bg-white/5`}>
                          {fren.rarity === 'Mythic' ? '🐉' : fren.rarity === 'Legendary' ? '🦄' : fren.rarity === 'Epic' ? '🦊' : fren.rarity === 'Rare' ? '🐶' : '🐹'}
                       </div>
                       <p className="text-center font-bold text-[10px] tracking-tight line-clamp-1">{fren.name}</p>
                       <p className="text-center text-[10px] text-white/40 font-mono mt-1">+{fren.productionRate} /s</p>
                       
                       <div className="absolute inset-x-0 bottom-3 px-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="w-full text-[10px] h-8 font-bold border border-white/20 bg-white/10 hover:bg-white/30 rounded-xl" onClick={() => handleMerge(fren)}>Merge 🧬</button>
                       </div>
                    </div>
                  </motion.div>
                ))}
...
function TokenStats() {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-[32px] p-5 flex items-center justify-between">
        <div>
          <p className="text-[10px] uppercase font-bold tracking-widest text-white/50">Treasury</p>
          <div className="flex items-center gap-2 mt-1">
             <span className="text-2xl">🪙</span>
             <p className="text-2xl font-black font-mono text-green-400">{state.frenTokens.toFixed(2)}</p>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 py-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-[0.2em] text-blue-400 font-bold opacity-80 mt-1">Base Ecosystem • ERC-8021 Active</p>
          </div>
        </div>
      ...
      <div className="mt-8 bg-white/5 backdrop-blur-lg border border-white/10 rounded-[32px] p-6">
         <h2 className="text-xs font-bold uppercase tracking-widest text-white/60 mb-4">Prompt Book for Art Assets</h2>
         <div className="space-y-4">
            <div className="p-4 bg-white/5 border border-white/5 rounded-2xl font-mono text-[10px] text-white/70">
               <span className="text-white/40 block mb-2">// Midjourney / Grok - Mascot / UI Elements</span>
               "A highly polished 3D icon of an adorable blob character in isometric view, holding a wrench, bright pastel colors, neon accents, Base blockchain blue ecosystem vibe, white background, suitable for a playful casual crypto clicker game UI, ultra high definition, octane render --v 6.0"
            </div>
            <div className="p-4 bg-white/5 border border-white/5 rounded-2xl font-mono text-[10px] text-white/70">
               <span className="text-white/40 block mb-2">// Midjourney / Grok - Fren Evolution (Legendary)</span>
               "Cute chibi cyberpunk pet mascot, glowing neon legendary aura, holographic tech wear, adorable big eyes, standing heroically, bright vivid colors on a clean dark background, 2d vector art style, clean lines, game asset ready, dynamic pose --v 6.0"
            </div>
            <div className="p-4 bg-white/5 border border-white/5 rounded-2xl font-mono text-[10px] text-white/70">
               <span className="text-white/40 block mb-2">// Midjourney / Grok - Resource Icon (Goo)</span>
               "A shiny, translucent drop of neon green goo resource item for a mobile game, glowing core, 3d glossy render, bright and saturated colors, isolated on white background, hyperdetailed, magical --v 6.0"
            </div>
         </div>
      </div>
    </div>
